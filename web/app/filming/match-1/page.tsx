...
// Match 1 Data - Mexico vs South Africa (Opening Match!)
const matchData = {
  matchNumber: 1,
  date: 'June 11, 2026',
  time: '3:00 PM ET',
  ...
  team1: {
    ...
    starPlayers: [
      ...
      { name: 'Santiago Giménez', position: 'Forward', club: 'Feyenoord' },
      { name: 'Hirving Lozano', position: 'Winger', club: 'PSV Eindhoven' },
    ],
  },
  team2: {
    name: 'South Africa',
    ...
    primaryColor: '#007A4D',
    starPlayers: [
      { name: 'Percy Tau', position: 'Winger', club: 'Al Ahly' },
      ...
      <div className="relative z-10 min-h-screen flex flex-col">
        <nav className="fixed top-0 left-0 right-0 bg-black/80 backdrop-blur-sm p-4 flex items-center justify-between z-50">
          <div className="flex items-center gap-4">
            <span className="text-green-500 font-bold">FILMING MODE</span>
            <span className="text-gray-400">Match {matchData.matchNumber} - OPENING MATCH</span>
          </div>
        ...
        <nav className="fixed bottom-0 left-0 right-0 bg-black/80 backdrop-blur-sm p-4 flex items-center justify-between">
          ...
          <button
            ...
            className="px-6 py-2 bg-green-600 rounded-lg disabled:opacity-30 hover:bg-green-700 transition-all"
          >
            Next →
          </button>
        </nav>
...
function IntroSection({ match }: { match: typeof matchData }) {
  return (
    <div className="text-center px-8 animate-fadeIn">
      <div className="mb-6">
        <span className="inline-block px-6 py-2 bg-green-600/80 rounded-full text-lg font-bold tracking-wider">
          🎉 OPENING MATCH • MATCH {match.matchNumber}
        </span>
      </div>
      ...
      <h2 className="text-4xl md:text-5xl font-bold mb-6">
        {match.team1.name} <span className="text-green-500">vs</span> {match.team2.name}
      </h2>
  ...
  return (
    <div className="w-full max-w-6xl mx-auto px-8 animate-fadeIn">
      <div className="grid md:grid-cols-2 gap-12 items-center">
        <div>
          <h2 className="text-2xl font-bold text-green-500 mb-4 tracking-wider">THE VENUE</h2>
          <h3 className="text-5xl md:text-6xl font-black mb-6">
            {match.stadium}
            <span className="block text-green-500 text-3xl mt-2">{match.city}</span>
          </h3>

          <p className="text-xl text-gray-300 mb-8 leading-relaxed">
            The legendary {match.stadium} hosts the opening match of the 2026 World Cup.
            This iconic venue has witnessed countless historic moments in football history.
          </p>

          <div className="grid grid-cols-2 gap-6">
            <div className="bg-white/10 rounded-lg p-4">
              <div className="text-3xl font-bold text-green-400">{match.capacity}</div>
              <div className="text-gray-400">Capacity</div>
            </div>
            <div className="bg-white/10 rounded-lg p-4">
              <div className="text-3xl font-bold text-green-400">{match.surface}</div>
        ...
        <div className="relative aspect-video rounded-xl overflow-hidden bg-gray-900">
          {stadiumImage && !imageError ? (
            ...
            <div className="absolute inset-0 flex items-center justify-center bg-gradient-to-br from-green-900/50 to-black">
              <div className="text-center">
...
function TeamProfileSection({ team }: { team: typeof matchData.team1 }) {
  return (
    <div className="text-center px-8 animate-fadeIn max-w-4xl mx-auto">
      ...
      {team.nickname && (
        <p className="text-2xl text-green-400 mb-8 italic">&ldquo;{team.nickname}&rdquo;</p>
      )}

      <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-8">
        <div className="bg-white/10 rounded-lg p-4">
          <div className="text-3xl font-bold text-green-400">#{team.fifaRanking}</div>
          <div className="text-gray-400 text-sm">FIFA Ranking</div>
        </div>
        <div className="bg-white/10 rounded-lg p-4">
          <div className="text-3xl font-bold text-green-400">{team.appearances}</div>
          <div className="text-gray-400 text-sm">WC Appearances</div>
        </div>
        <div className="bg-white/10 rounded-lg p-4">
          <div className="text-xl font-bold text-green-400">{team.bestFinish}</div>
          <div className="text-gray-400 text-sm">Best Finish</div>
        </div>
        <div className="bg-white/10 rounded-lg p-4">
          <div className="text-xl font-bold text-green-400">{team.coach}</div>
...
function PlayersSection({ team }: { team: typeof matchData.team1 }) {
  return (
    <div className="text-center px-8 animate-fadeIn max-w-5xl mx-auto">
      <h2 className="text-3xl font-bold text-green-500 mb-2 tracking-wider">KEY PLAYERS</h2>
      ...
      <div className="grid md:grid-cols-3 gap-8">
        {team.starPlayers.map((player, idx) => (
          <div key={idx} className="bg-white/10 rounded-xl p-6 text-left">
            ...
            <p className="text-green-400 text-lg mb-1">{player.position}</p>
...
function PreviewSection({ match }: { match: typeof matchData }) {
  return (
    <div className="text-center px-8 animate-fadeIn max-w-4xl mx-auto">
      <h2 className="text-2xl font-bold text-green-500 mb-4 tracking-wider">MATCH PREVIEW</h2>

      <h3 className="text-4xl md:text-5xl font-black mb-8">
        The World Cup 2026 kicks off in {match.city}!
      </h3>

      <div className="w-24 h-1 bg-green-600 mx-auto mb-8" />

      <p className="text-xl text-gray-300 leading-relaxed mb-8">
        Host nation {match.team1.name} takes on {match.team2.name} in a historic opening match
        at the legendary {match.stadium}. The atmosphere will be electric as fans from
        around the world gather to witness the start of football&apos;s greatest tournament.
      </p>
...
function EndCardSection({ match }: { match: typeof matchData }) {
  return (
    <div className="text-center px-8 animate-fadeIn">
      <div className="mb-8">
        <span className="inline-block px-6 py-2 bg-green-600 rounded-full text-lg font-bold tracking-wider">
          OPENING MATCH • MATCH {match.matchNumber}
        </span>
      </div>
