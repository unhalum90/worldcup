...
interface ItineraryOption {
  ...
  cities: Array<{
    cityName: string;
    nights?: number; // Number of nights staying in this city
    lodgingZones: Array<{
      zoneName: string;
      whyStayHere: string;
...
export default function ItineraryResults({ itinerary, onEmailCapture }: ItineraryResultsProps) {
  ...
  // Helper to calculate total nights from itinerary
  const calculateTotalNights = (cities: ItineraryOption['cities']): number => {
    return cities.reduce((total, city) => {
      return total + (city.nights || 2); // Default to 2 nights if not specified
    }, 0);
  };

  // Helper to calculate weighted lodging cost based on actual nights per city
  const calculateLodgingCost = (cities: ItineraryOption['cities']): number => {
    let totalCost = 0;
    cities.forEach(city => {
      const nightsInCity = city.nights || 2;
      const ratePerNight = parseCost(city.lodgingZones[0]?.estimatedRate, 200);
      totalCost += ratePerNight * nightsInCity;
    });
      ...
      {expandedOption !== null ? (
        <div className="min-h-screen">
          ...
          <button
            ...
            className="fixed top-6 right-6 z-50 bg-white hover:bg-gray-100 text-gray-700 rounded-full p-3 shadow-xl border-2 border-gray-300 transition-all hover:scale-110"
            aria-label="Close and return to options"
          >
          ...
          {/* Full option details */}
          {(() => {
            const option = itinerary.options[expandedOption];
            
            return (
              <div className="space-y-8 pb-8">
                {/* Option Header */}
                <div className="bg-gradient-to-r from-blue-600 to-blue-700 p-8 rounded-xl shadow-lg">
                ...
                {/* Option Content */}
                <div className="bg-white border-2 border-gray-200 rounded-xl shadow-xl p-8 space-y-8">
                  {/* Route Ribbon */}
                  ...
                  {/* Cost Summary */}
                  <div className="bg-gradient-to-r from-blue-50 to-sky-50 border-2 border-blue-300 rounded-xl p-6 shadow-lg">
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex justify-between items-center py-2 border-b border-blue-200">
                        ...
                        <span className="font-bold text-blue-800 text-lg">
                          ${calculateLodgingCost(option.cities).toLocaleString()}
                        </span>
                      </div>
                      ...
                      <div className="flex justify-between items-center pt-4 mt-2 border-t-2 border-blue-400">
                        ...
                        <span className="text-2xl font-bold text-blue-900">
                          {(() => {
                            ...
                            const lodgingCost = calculateLodgingCost(option.cities);
                      ...
                      <p className="text-xs text-gray-600 mt-4 italic">
                        * Estimates based on {calculateTotalNights(option.cities)} nights total across {option.cities.length} {option.cities.length === 1 ? 'city' : 'cities'}, mid-range accommodations, 
        ...
        /* Collapsed view - Accordion pills */
        <div className="space-y-6">
          <div className="text-center space-y-4">
            <h1 className="text-4xl font-bold text-gray-900">Your World Cup 2026 Itinerary Options</h1>
            <p className="text-lg text-gray-600">We've created {itinerary.options.length} personalized trip plans for you. Click any option to view details.</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {itinerary.options.map((option, index) => {
              ...
              const transportCost = option.cities.length * 150;
              const totalCost = flightCost + lodgingCost + transportCost;
              const totalNights = calculateTotalNights(option.cities);
              ...
              return (
                <button
                  key={index}
                  onClick={() => setExpandedOption(index)}
                  className={`group bg-white border-2 ${colors.border} rounded-xl p-6 shadow-lg hover:shadow-2xl ${colors.hoverBorder} transition-all transform hover:scale-105 text-left`}
                >
                  {/* Option badge */}
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <svg className={`w-5 h-5 text-gray-400 ${colors.hoverIcon} transition-colors`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
                    </svg>
                  </div>

                  {/* Title */}
                  <h3 className={`text-xl font-bold text-gray-900 mb-4 ${colors.hoverText} transition-colors`}>
                    {option.title}
                  </h3>
                  ...
                  <div className="space-y-2 mb-4">
                    <div className="flex items-center text-sm text-gray-600">
                      ...
                      <span>{option.cities.length} {option.cities.length === 1 ? 'city' : 'cities'}</span>
                    </div>
                    <div className="flex items-center text-sm text-gray-600">
                      <span className="mr-2">✈️</span>
                      <span>{option.flights.legs?.length || 1} {(option.flights.legs?.length || 1) === 1 ? 'flight' : 'flights'}</span>
                    </div>
                    <div className="flex items-center text-sm text-gray-600">
                  ...
                  <div className="pt-4 border-t border-gray-200">
                    <div className="flex items-center justify-between">
                      ...
                      <span className={`text-xl font-bold ${colors.costText}`}>
                  ...
                  {/* Click hint */}
                  <p className="text-xs text-gray-500 mt-4 text-center">Click to view full details</p>
                </button>
              );
            })}
          </div>
